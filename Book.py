...
def main(files):
    ...
    while (remainingDays > 0 and i != noOfLibraries):
        ...
        print("SDays: {0} TotScore: {1} SC: {3} QF: {2}  RDays: {4} ".format(libraries[i].signupDays, libraries[i].totBookScore, libraries[i].qualityFactor, libraries[i].shippingCapacity, remainingDays, i))
...
if __name__ == "__main__":
    score=int(0)
    filelist = ["a_example.txt","b_read_on.txt", "c_incunabula.txt", "d_tough_choices.txt", "e_so_many_books.txt", "f_libraries_of_the_world.txt"]
